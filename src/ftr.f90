...
subroutine find_chi2_tr_nuevo(xd,yd,errs,trlab,jtrlab,pars,rps,ldc,jtr,flag, &
...
!Local variables
  ...
  if ( is_good ) then
    ...
    call flux_tr_new(xd,trlab,up_pars,rps,up_ldc,&
...
subroutine flux_tr_new(xd,trlab,pars,rps,ldc,&
           n_cad,t_cad,nbands,datas,npl,muld)
implicit none
  ...
  do j = 0, datas - 1
    ...
    do n = 0, npl - 1
      ...
      if ( ALL( z > 1.d0 + rps(trlab(j),n) ) .or. rps(trlab(j),n) < small ) then
        ...
        !Now we have z, let us use Agol's routines
        call occultquad(z,u1(trlab(j)),u2(trlab(j)),rps(trlab(j),n),flux_ub(:,n),mu,n_cad)
        !!!!!call qpower2(z,rp(n),u1,u2,flux_ub(:,n),n_cad)
