...
frv.metropolis_hastings_rv(mega_time,mega_rv,mega_err,tlab,v0,k0,e0,w0,T0,Porb,prec,imcmc,tfactor,chi_toler,is_circular)
...
for i in range(1,n):
	rvx[i] = rvx[i-1] + dn
if ( is_circular ):
	rvy = frv.rv_circular(rvx,0.0,T0,kval,Porb)
else:
	rvy = frv.rv_curve(rvx,0.0,T0,kval,Porb,ecval,wval)

res = [None]*nt
for i in range(0,nt):
	if (is_circular):
		res[i] = frv.rv_circular(time_all[i],0.0,T0,kval,Porb)
	else:
		res[i] = frv.rv_curve(time_all[i],0.0,T0,kval,Porb,ecval,wval)
